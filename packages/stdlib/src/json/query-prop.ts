 ...
 */
export function queryPropPath<R, T>(json: T, path: PropertyKey[] | string): R | null {
  if (json == null) {
    return null;
  }
  ...
  return (path.reduce((innerObj, nextProperty) => (innerObj as never)?.[nextProperty], json) as unknown as R) ?? null;
}
 ...
 */
export function queryProp<R, T>(json: T, query: ((obj: T) => unknown) | PropertyKey[] | string): R | null {
  return typeof query !== 'function' ? queryPropPath<R, T>(json, query) : (query(json) as unknown as R) ?? null;
}
